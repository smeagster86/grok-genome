...
export function CategoryCard({ category, insights, isActive, onClick }: Props) {
  ...
  const highImpact = insights.filter(i => 
    i.interpretation.effect === 'increased_risk' || 
    i.interpretation.effect === 'non_responder' || 
    i.interpretation.effect === 'affected'
  ).length;
    ...
    >
      ...
      <div>
        ...
        <div className="text-xs text-white/60">
          {highImpact > 0 ? `${highImpact} flagged for attention` : 'All variants within typical range'}
        </div>
      </div>

      <div className="mt-auto pt-2 text-[11px] text-white/40 flex items-center gap-1">
        Tap to filter explorer →
      </div>
    </motion.button>
  );
}
