...
export function CategoryCard({ category, insights, isActive, onClick }: Props) {
  ...
  const impactPercent = count > 0 ? Math.round((highImpact / count) * 100) : 0;
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <div className="text-xs text-white/60">{highImpact > 0 ? `${highImpact} flagged for attention` : 'Mostly typical range'}</div>
      </div>
      ...
      <div className="mt-1 h-1.5 w-full bg-white/10 rounded-full overflow-hidden">
        <div 
          ...
          style={{ 
            width: `${impactPercent}%`, 
            backgroundColor: highImpact > 0 ? '#ef4444' : '#10b981' 
          }} 
        />
      </div>
